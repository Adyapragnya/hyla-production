...
const MyMapComponent = ({ vessels, selectedVessel, setVesselEntries }) => {
  ...
  useEffect(() => {
    const fetchGeofences = async () => {
      try {
        ...
        const response = await axios.get(`${baseURL}/api/polygongeofences`);
        const polylineResponse = await axios.get(`${baseURL}/api/polylinegeofences`);
        const circleResponse = await axios.get(`${baseURL}/api/circlegeofences`); // Fetch circle geofences
  ...
  return (
    ...
    <MapContainer center={[0, 0]} minZoom={1.5} zoom={1.5} maxZoom={18} style={{ height: '55vh', width: '100%', backgroundColor: 'rgba(170,211,223,255)'}}>
  <TileLayer
